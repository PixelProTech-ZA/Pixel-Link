...
function viewSettings() {
  return `<div class="view">
    ...
    <div class="card" id="settings-install-card" style="display:none;">
      <div class="section-title"><h2>Install</h2></div>
      <p style="font-size:12.5px;color:var(--text-faint);margin:8px 0 12px;">Install Pixel Link as an app for faster access and an offline-ready window.</p>
      <button class="btn btn-secondary btn-block" data-action="install-app">${ic('download', 15)} Install app</button>
    </div>
    <div class="card">
...
window.addEventListener('appinstalled', () => {
  ...
  appInstalled = true;
  toast('Pixel Link installed');
  renderInstallUI();
});

function renderInstallUI() {
  const btn = document.getElementById('topbar-install-btn');
  if (btn) btn.style.display = (deferredInstallPrompt && !appInstalled) ? '' : 'none';
  const card = document.getElementById('settings-install-card');
  if (card) card.style.display = (deferredInstallPrompt && !appInstalled) ? '' : 'none';
}
...
async function promptInstall() {
  ...
  if (outcome === 'accepted') toast('Installing Pixel Link…');
  deferredInstallPrompt = null;
  renderInstallUI();
}
...
async function bindView() {
  ...
  // settings
  bindOnce('install-app', () => promptInstall());
